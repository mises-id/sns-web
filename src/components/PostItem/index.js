 ...
 * @LastEditTime: 2021-08-29 16:08:52
 * @LastEditors: lmk
 * @Description: 
 */
...
const PostItem = ({val={},index,history,changeFollow,setLike,btnType,deleteItem,type})=>{
  ...
  {val.content&&<p className={`itemContent  ${type!=='detail' ? 'item-eli m-font15 m-margin-tb10' : 'm-font18 m-margin-tb15 detail-content'}`}>{val.content}</p>}
  ...
  {val.parent_status&&<div className="m-bg-f8f8f8 m-padding10 border-radius5" 
    ...
    {val.parent_status.content&&<p className="itemContent m-font13 m-margin-tb10">{val.parent_status.content}</p>}
